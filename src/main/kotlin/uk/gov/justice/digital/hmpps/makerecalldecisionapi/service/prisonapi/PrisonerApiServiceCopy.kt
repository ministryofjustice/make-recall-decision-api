...
@Service
internal class PrisonerApiServiceCopy(
  ...
  /**
   * Sort sentences by first the sentence end date and then by court if there are any with the same date
   */
  ...
  fun retrieveOffences(nomsId: String): List<SentenceSequence> {
    ...
    val licenceExpiryDate = offender?.sentenceDetail?.licenceExpiryDate

    val offenderPrisonTimelines = getValueAndHandleWrappedException(
      prisonApiClient.retrievePrisonTimelines(nomsId),
    )!!
    return offenderPrisonTimelines.prisonPeriod
      .flatMap { prisonPeriod ->
        val sentencesForBooking = sentencesForPeriodBooking(prisonPeriod, licenceExpiryDate)

        val sentenceSequenceMap = groupSentencesIntoSequences(sentencesForBooking)
  ...
  fun sentencesForPeriodBooking(prisonPeriod: PrisonPeriod, licenceExpiryDate: LocalDate?): List<Sentence> {
    val (prisonDescription, dateOutOfPrison) = prisonDepartureInfo(prisonPeriod)
  ...
  fun prisonDepartureInfo(prisonPeriod: PrisonPeriod): Pair<String?, LocalDateTime?> {
    ...
    val movementToLeavePrison = prisonPeriod.movementDates.find { it.dateOutOfPrison === lastDateOutOfPrison }
    val prisonDescription = movementToLeavePrison?.releaseFromPrisonId?.let {
      try {
        prisonApiClient.retrieveAgency(movementToLeavePrison.releaseFromPrisonId).block()?.longDescription
      } catch (notFoundEx: NotFoundException) {
        log.info("Agency with id ${movementToLeavePrison.releaseFromPrisonId} not found: ${notFoundEx.message}")
        null
      }
    }

    return Pair(prisonDescription, lastDateOutOfPrison)
  }

  fun groupSentencesIntoSequences(sentencesForBooking: List<Sentence>): MutableMap<Int, SentenceSequence> {
    val sentenceSequenceMap: MutableMap<Int, SentenceSequence> = initialiseSentenceSequenceMap(sentencesForBooking)

    // Any sentence that is not an index must be a consecutive/concurrent
    val consecutiveSentences = sentencesForBooking.filter { it.consecutiveToSequence !== null }
    ...
    val consecutiveSentencesBySequence = consecutiveSentences.groupBy { it.consecutiveToSequence!! }.toSortedMap()
    // For every consecutiveToSequence (which we know already contains any concurrent for that consecutiveToSequence
    consecutiveSentencesBySequence.forEach { (consecutiveTo, sentences) ->
      if (sentenceSequenceMap.containsKey(consecutiveTo)) {
        startSequence(sentenceSequenceMap, consecutiveTo, sentences)
      } else {
        appendToSequence(sentenceSequenceMap, consecutiveTo, sentences)
      }
    }

    return sentenceSequenceMap
  }

  fun initialiseSentenceSequenceMap(sentencesForBooking: List<Sentence>): MutableMap<Int, SentenceSequence> {
  ...
  private fun startSequence(
    sentenceSequenceMap: MutableMap<Int, SentenceSequence>,
    consecutiveTo: Int,
