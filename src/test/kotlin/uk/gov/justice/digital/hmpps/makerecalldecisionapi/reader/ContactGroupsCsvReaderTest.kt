...
class ContactGroupsCsvReaderTest {
  ...
  fun `given a csv file then read the contents and map to list of contact groups`() {
    ...
    assertEquals("2", result.get(14).groupId)
    assertEquals("Anti-terrorism", result.get(14).groupName)
    assertEquals("PREVENT", result.get(14).code)
    assertEquals("Prevent Activity", result.get(14).contactName)
  }
}
